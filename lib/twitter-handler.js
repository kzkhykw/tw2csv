...
module.exports =
  class twitterAPI {
    ...
    searchTweets (queryArg, outputFileName, nextResultsMaxIdArg = null , countArg = 100 , searchMode) {
      let _this = this; // avoid "undefined" in `this.client.get()`
      this.client.get('search/tweets', { q: queryArg, count: countArg, lang:"ja", max_id: nextResultsMaxIdArg }, function (error, searchData, response) {
        for (let item in searchData.statuses) {
          ...
          let userIsBot = !utils.isHuman(twitterClientMatch)
          let labelEmoji = '🐦'
          ...
          if (searchMode == 0) {
            if (!tweetIsRT && !userIsBot) {
              csv.dataAppend(outputFileName, pushingArray, labelEmoji)
            }
        ...
        if (searchData.search_metadata == undefined) {
          ...
          _this.searchTweets(queryArg, outputFileName, maxId[1], countArg, searchMode)
        } else {
